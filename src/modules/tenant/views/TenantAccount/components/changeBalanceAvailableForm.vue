...
const balanceAvailableModel = ref<{
  ...
  change_balance: 0,
  change_balance_max: formType === 'sub' ? Number(data?.balance_available) : 99999,
...
useForm('changeBalanceAvailableForm').then(async (form: MaFormExpose) => {
  ...
  form.setItems([
    ...
    {
      label: formType === 'add' ? t('tenantAccount.add') : t('tenantAccount.sub'),
      prop: 'change_balance',
...
function add(): Promise<any> {
  return new Promise((resolve, reject) => {
    if (data?.id) {
      changeBalanceAvailable(data.id as number, balanceAvailableModel.value.change_balance).then((res: any) => {
