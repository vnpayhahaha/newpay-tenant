...
const balanceFrozenModel = ref<{
  ...
  change_balance: 0,
  change_balance_max: formType === 'unfreeze' ? Number(data?.balance_frozen) : Number(data?.balance_available),
...
useForm('changeBalanceFrozenForm').then(async (form: MaFormExpose) => {
  ...
  form.setItems([
    ...
    {
      label: formType === 'freeze' ? t('tenantAccount.freeze') : t('tenantAccount.unfreeze'),
      prop: 'change_balance',
...
function freeze(): Promise<any> {
  return new Promise((resolve, reject) => {
    if (data?.id) {
      changeBalanceFrozen(data.id as number, balanceFrozenModel.value.change_balance).then((res: any) => {
