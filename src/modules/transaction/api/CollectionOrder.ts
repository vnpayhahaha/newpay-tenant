...
export function cancel(ids: number[]): Promise<ResponseStruct<null>> {
  return useHttp().put("/tenant/transaction/collection_order/cancel", {
    data: ids,
  });
}

//   submitted_utr: 提交UTR
export function submitted_utr(id: number, submitted_utr: string): Promise<ResponseStruct<null>> {
  return useHttp().put(`/tenant/transaction/collection_order/submitted_utr/${id}`, {
    data: submitted_utr,
  });
}
